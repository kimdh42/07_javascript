...
// const x;     // 선언만 하면 오류 발생
const x = 1;

// 2. const 키워드로 선언한 변수는 재할당이 금지된다.
// x = 2;      // TypeError: Assignment to constant variable.(재할당 금지)


// 할인율은 변결할 수 없는 상수로서 사용 될 값
...
/* 
    ...
    1. 모든 코드가 전역 변수를 참조하고 변경할 수 있는 "암묵적 결합"을 허용한다.
       변수의 유효 번위가 크면 클수록 코드 가독성은 나빠지고 의도치 않게
       상태가 변경될 수 있는 위험성이 높다.
    2. 전역 변수는 "생명주기가 길다"
       메모리 리소스도 오래 소비하며, 상태 변경이 간으한 시간 및 기회가 많다.
       더욱이 중복 된 변수명을 통해 의도치 않은 재할당이 일어날 수도 있다.
    3. "스코프 체인 상에서 종점에 존재" 하므로 전역 변수의 검색속도가 가장 느리다.
    4. "네임 스페이스가 오염" 된다.
       자바스크립트는 파일이 분리 되어 있얻도 하나의 전역 스코프를 공유하므로 예상치 못한 결과를 가져올 수 있다.
